...
(function() {
	...
	columns.push('Time');
	columns.push('Model_1');
	columns.push('Model_2');
	columns.push('Model_3');
	columns.push('Model_4');
	columns.push('Model_5');
	columns.push('Model_6');
    columns.push('Model_7');
	...
	if (data[0].length > 2){
            data[0][1]= process(data[0][1]);
	}
        wso2gadgets.onDataReady(data,"append");

	$('td span').each(function(){
		if ($(this).text() === 'normal') {
		    $(this).parent().css('background-color','#1f77b4');
		}else if ($(this).text() === 'anomaly') {
		    $(this).parent().css('background-color','#ff7f0e');
		}
	});

	$('td span').each(function(){
		if ($(this).text() === 'Normal') {
		    $(this).parent().css('background-color','#2ca02c');
		}else if ($(this).text() === 'Anomaly') {
		    $(this).parent().css('background-color','#ff7f0e');
		}
	});
    ...
    process = function(date_millis){
        var date= new Date(parseInt(date_millis));
